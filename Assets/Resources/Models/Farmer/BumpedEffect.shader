Shader "Custom/BumpedEffect"
{
    Properties
    {
		...
		_DispFactor("Displacement factor", Float) = 1.0
    }
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
			...
			float nse = inoise(float3(IN.uv_MainTex * _GridSize, 0.0))* _DispFactor;

			float nse_x = inoise(float3((IN.uv_MainTex.xy + float2(0.03, 0)) * _GridSize, 0.0))* _DispFactor;
			float nse_y = inoise(float3((IN.uv_MainTex.xy + float2(0, 0.03)) * _GridSize, 0.0))* _DispFactor;
			o.Normal = normalize(float3(-(nse_x - nse), -(nse_y - nse),1));
